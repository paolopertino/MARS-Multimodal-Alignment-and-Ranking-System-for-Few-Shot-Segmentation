...
class SamAutomaticMaskGenerator:
    def __init__(
        ...
        # Used to cache features for the model for cascaded post-refinement steps
    ...
    def generate(
        ...
        for idx in range(len(mask_data["segmentations"])):
            ann = {
                ...
                "point_coords": mask_data["points"][idx], # [mask_data["points"][idx].tolist()],
    ...
    def _generate_masks(
        ...
        crop_boxes, layer_idxs, crop_box_ids = generate_crop_boxes(
            orig_size, self.crop_n_layers, self.crop_overlap_ratio
        )

        # Iterate over image crops
        data = MaskData()
        for crop_box, layer_idx, box_id in zip(crop_boxes, layer_idxs, crop_box_ids):
            if layer_idx > 0:
                crop_data = self._process_crop(image, crop_box, layer_idx, orig_size, box_id=box_id)
            else:
                crop_data = self._process_crop(
                    ...
                    select_point_labels,
                    select_box,
                    [select_mask_input] if select_mask_input is not None else None,
                    box_id=box_id
                )
            data.cat(crop_data)
    ...
    def _process_crop(
        ...
        # Caching features for cascaded post-refinement steps (the image/crop features remain the same for all the cascaded steps)
        if box_id is not None and self.stored_features.get(box_id) is not None:
            self.predictor.set_image(cropped_im, stored_features=self.stored_features[box_id])
        else:
            self.predictor.set_image(cropped_im)
            self.stored_features[box_id] = self.predictor.features.clone().cpu().numpy()
        ...
        if crop_layer_idx == 0 and select_point_coords is not None:
            ...
            for select_point_coords, select_point_labels, select_box, select_mask_input in \
                ...
                # sel_points.shape = 64 x num_points x 2 | sel_labels.shape = 64 x num_points
    ...
    def _process_batch(
        ...
        data = MaskData(
            ...
            points=torch.as_tensor(points.repeat(masks.shape[1], axis=0)[:, None, :]).tolist(),
    ...
    def _process_sel_batch(
        ...
        if point_coords is not None:
            ...
            original_point_coords = point_coords.copy()
        ...
        data = MaskData(
            ...
            iou_preds=iou_preds.flatten(0, 1),
            low_res_masks=low_res_masks.flatten(0, 1),
            points=torch.as_tensor(original_point_coords.repeat(masks.shape[1], axis=0)).tolist(),
        )
        del masks
