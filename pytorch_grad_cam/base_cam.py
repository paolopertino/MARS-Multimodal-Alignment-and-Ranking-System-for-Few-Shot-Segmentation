...
class BaseCAM:
    ...
    def forward(self,
        ...
        # outputs is a tuple (logits_per_image, attn_weights), where logits per image are softmaxed
        outputs = self.activations_and_grads(input_tensor, H, W)
        if targets is None:
            if isinstance(input_tensor, list):
                target_categories = np.argmax(
                    outputs[0].cpu().data.numpy(), axis=-1)
            else:
                target_categories = np.argmax(
                    outputs.cpu().data.numpy(), axis=-1)
