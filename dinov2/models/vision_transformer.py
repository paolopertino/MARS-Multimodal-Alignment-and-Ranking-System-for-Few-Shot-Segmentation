...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_features(self, x, masks=None):
        ...
        # BS x 1+(num_registers)+H*W x C, the 1 stands for the cls token.
        x = self.prepare_tokens_with_masks(x, masks)
        # num_registers are the number of regs used. They're placed after the cls token.
        # H = H_original_img / patch_size,
        # W = W_orig_img / patch_size
    ...
    def get_last_self_attention(self, x, masks=None):
        ...
        for i, blk in enumerate(self.blocks):
            if i < len(self.blocks) - 1:
                block_attention = blk(x, return_attention=True)
                attention_maps += (block_attention, )
