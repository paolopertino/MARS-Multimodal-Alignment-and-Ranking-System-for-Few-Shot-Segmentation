...
class Matcher:
    ...
    def predict(self):
        ...
        all_points, box, S, C, reduced_points_num = self.patch_level_matching(
            ref_feats=ref_feats, tar_feat=tar_feat)
        points = self.clustering(
            all_points) if not self.use_points_or_centers else all_points
        self.set_rps()
        pred_masks = self.mask_generation(
            self.tar_img_np, points, box, all_points, self.ref_masks_pool, C)
        return pred_masks
    ...
    def extract_img_feats(self):
        ...
        ref_feats = self.encoder.forward_features(
            ref_imgs.to(self.device))["x_prenorm"][:, 1:]
    ...
    def patch_level_matching(self, ref_feats, tar_feat):
        ...
        if not (retain_ind == False).all().item():
            indices_forward = [indices_forward[0]
    ...
    def mask_generation(self, tar_img_np, points, box, all_ponits, ref_masks_pool, C):
        ...
        for point in samples_list:
            ...
            pred_masks, ious = self.generator.predict(
                self.tar_img_np, self.tar_images, point)
            pred_masks = F.interpolate(
                pred_masks[None, ...], self.input_size, mode="bilinear", align_corners=False)
            tar_masks_ori.append(pred_masks[0])
        tar_masks = torch.cat(tar_masks_ori, dim=0)[
            :, None, ...].cpu().numpy() > 0
...
def build_matcher_oss(args):
    ...
    dinov2 = vits.__dict__[args.backbone_size](**dinov2_kwargs)

    dinov2_utils.load_pretrained_weights(
        dinov2, args.backbone_weights, "teacher")
    dinov2.eval()
