        ...
        if return_attn:
            ...
            for i, block in enumerate(self.resblocks):
                if i == len(self.resblocks) - 1:
                    x, attn_final = block(x, return_attn=True)
                    attention_layers.append(attn_final)
                    layers_output.append(x)
                    return layers_output, attention_layers
                else:
                    x, attn_i = block(x, return_attn=True)
                    attention_layers.append(attn_i)
                    layers_output.append(x)
            assert False
        return self.resblocks(x)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, alpha=None, return_attn=False):
        ...
        if return_attn:
            ...
            x = [self.ln_post(i[:, 0, :]) for i in x]
            x = [i @ self.proj for i in x]
            return x, attn_last
        else:
    ...
    def forward_features(self, x: torch.Tensor, alpha=None):
        x = self.conv1(x)
        x = x + self.conv1_alpha(alpha)
        ...
        x = x.permute(1, 0, 2)
        x, attn_last_layer = self.transformer(x, return_attn=True)
        
        x_prenorm_no_proj = x.permute(1, 0, 2)
        x_norm = self.ln_post(x_prenorm_no_proj[:, :, :]) # Take only CLS token
        
        if self.proj is not None:
            x_proj = x_norm @ self.proj
            x_prenorm_proj = x_prenorm_no_proj @ self.proj
        
        return {
            'x_prenorm_no_projection': x_prenorm_no_proj,
            'x_norm': x_norm,
            'x_proj': x_proj,
            'cls_token': x_proj[:, 0, :],
            'x_prenorm_projected': x_prenorm_proj,
            'attention_last_layer': attn_last_layer
        }
