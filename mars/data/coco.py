r""" COCO-20i few-shot semantic segmentation dataset. Adapted from Matcher."""
import os
import pickle
...
class DatasetCOCO(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, use_original_imgsize):
        ...
        self.idx_to_classname = {
            i: COCO_ID_LABELS_MAPPING[i] for i in self.class_ids}
        self.img_metadata_classwise = self.build_img_metadata_classwise()
        self.img_metadata = self.build_img_metadata()

    def __len__(self):
        # len(self.img_metadata) if self.split == 'trn' else 1000
        return 3000 if self.split == 'trn' else 1000
    ...
    def __getitem__(self, idx):
        ...
        if not self.use_original_imgsize:
            query_mask = F.interpolate(query_mask.unsqueeze(0).unsqueeze(
                0).float(), query_img.size()[-2:], mode='nearest').squeeze()

        support_imgs = torch.stack(
            [self.transform(support_img) for support_img in support_imgs])
        for midx, smask in enumerate(support_masks):
            support_masks[midx] = F.interpolate(smask.unsqueeze(0).unsqueeze(
    ...
    def load_frame(self):
        ...
        query_img = Image.open(os.path.join(
            self.base_path, query_name)).convert('RGB')
        ...
        for support_name in support_names:
            support_imgs.append(Image.open(os.path.join(
                self.base_path, support_name)).convert('RGB'))
